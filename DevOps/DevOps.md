...
* 基礎設施即代碼  
基礎設施即代碼這個其實是大陸用語，我能理解他的意思，但是我找不到比較合適的簡短文字解釋。隨著網路設備跟網路技術持續進步與更新。更加複雜的應用程式部署方式。如何有效的管理這些設備與有效的管理介面變成Ops的困擾。  
...
但是要讓QA團隊可以獨立運作，包含學習瞭解系統架構、環境建置、配置管理還要跟上。  
除此之外好記性不如爛筆頭，溝通要時間，描述問題要時間。講過的東西會忘，寫下來的不會，所以與其花很多時間溝通，不如花時間訓練QA寫文件。Issue Tracking…等等還有很多東西要做。  
...
另外，因為規格文件可以被執行，所以可以自動驗證規格與實作在多次修改之後是否依舊保持一致。也因為如此，所以和傳統開方法方式相比，會有比較低的bug比率。TDD只是恰巧可以扮演「自動測試」的工作，例如驗證程式運作是否符合預期，驗證有沒有改 A 壞 B 的情況，再透過測試覆蓋的路徑來觀察是否多寫了不必要的程式碼，或是遺漏了描述需求的測試案例。  
